...
export var _attack_distance : float = 60
...
var _bullet = preload("res://Enemy/EnemyFloaterOrb.tscn")
...
func _physics_process(delta):
	...
	if Global._player != null and !_is_attacking:
		...
		if _vision.get_collider() == Global._player:
			_target_position = _vision.get_collision_point() + (global_position - _vision.get_collision_point()).normalized() * _attack_distance*0.9
			
			if transform.origin.distance_to(_target_position) < _attack_distance and _attack_timer.time_left == 0:
				_attack_start()
...
func _attack_trigger():
	_velocity += 200 * Vector2.RIGHT.rotated(_vision.rotation+PI)
	_attack_timer.start()
	var _bullet_instance = Global.instance_node(_bullet, global_position, Global._node_creation_parent)
	_bullet_instance._fire(Vector2.RIGHT.rotated(_vision.rotation))
